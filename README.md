...
| Data | `list` | Read-only listing of system definition data: surfaces, asphere coefficients, thickness differences and glasses, plus paraxial properties and ray trace results. Flags: `--format table\|yaml\|json\|csv`, `--config`, `--glass-dir`, `--curvature`, `--all`, `--roles`, `--summary`. See `docs/list.md`. | YAML system → formatted table / yaml / json / csv |
