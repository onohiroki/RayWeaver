...
| Data | `list` | Read-only listing of system definition data: surfaces, asphere coefficients, thickness differences and glasses, plus paraxial properties and ray trace results. Flags: `--format table\|yaml\|json\|csv`, `--config`, `--glass-dir`, `--curvature`, `--all-glasses`, `--roles`, `--summary`. See `docs/list.md`. | YAML system → formatted table / yaml / json / csv |
| Propagation | `trace` | Trace individual rays and report per-surface intersection data (low-level). `trace single` traces one ray from CLI flags (`--origin`, `--direction`, `--aim`, `--angle-yz`, `--pass-through`, `--path`, `--details`, `--verbose`), needs no `rays` section, and pipes into `plot`/`list rays`. | rays → per-surface results |
| Propagation | `chief` | Sample the beam for each field: chief ray, dynamic pupil (per-field entrance/exit pupil from the chief-ray crossings when no `stop_surface` is set), pupil grid, marginal rays, spot statistics and OPL. Flags: `--clear-aperture` (size `auto_aperture: true` surfaces to the beam footprint + margin; `--clear-aperture-margin-mm`, `--clear-aperture-rays`), `--preserve-rays` (keep the existing rays section during aperture adjustment), `--marginal-rays`, `--pass-through N`, `--config ID`, `--wl`, `--ray-fan`/`--fan-plane`/`--fan-rotation`, virtual entrance pupil (`--epz Z`, `--epd D`, `--fnum N`, or the YAML `pupil_model` section). | system + fields → chief_rays / grid |
...
| Tooling | `query` | YAML/JSONL selector with in-memory edits: extract values, iterate arrays, aggregate, evaluate expressions, pass-gates and yq-style `--edit` mutations (`.`, `.key`, `PATH = VALUE`, `PATH \|= EXPR`, `del`, `+=`/`-=`) from a shell pipeline. Replaces `python3 + PyYAML` / `yq` in the sample demos. See `docs/query.md`. | YAML/JSONL → plain text / YAML / JSON / CSV |
...
| [docs/list.md](docs/list.md) | surfaces, glasses, asphere coefficients, thickness differences, paraxial properties, ray trace results |
| [docs/trace.md](docs/trace.md) | low-level ray tracing, `trace single` for a single ray from CLI flags |
...
# or on the 6-element lens (slow): bash samples/escape-demo.bash --lens 6elements
