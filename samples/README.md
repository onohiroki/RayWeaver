...
| `escape-demo.yaml` | Degraded US2645157 triplet (auto-aperture surfaces) with the escape-function merit: `spot_rms` (on-axis, weight 2.0), `spot_rms_t`/`spot_rms_s`/`spot_rms_worst` plus a 16° `wavefront_astigmatism` (weight 14000, target 0) on the 16° field, and `spot_rms_worst`/`spot_rms_weighted`/`spot_ee_radius` (weight 0.05) on the 24° field. Input of `escape-demo.bash` (triplet mode). The 16° astigmatism term is driven by a **DLS re-optimisation** after the escape run (see `psf-mtf-demo.yaml`); the 24° `wavefront_rms_residual` alternative was dropped because its frozen-pupil evaluation diverged from the chief-measured spot in DLS. |
