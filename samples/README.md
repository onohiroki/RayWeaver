...
| `my-schmidt.yaml` | DLS+escape-optimised folded D=200 F/1.93 Schmidt camera (the default `psf-mtf-demo.bash` lens): BK7 corrector plate + spherical primary + aspheric 2-element field flattener, evaluated at its usable fields 0/1/2/2.8° (the legacy 3.22° full-frame edge vignettes and is dropped). Cleaned copy of the P3 escape-optimisation adopted best (index 15) — only the sections `psf` consumes (glass_catalog / configs / chief) are kept. Reports MTF up to `psf.mtf_config.max_frequency` (100 c/mm). |
...
| `psf-mtf-demo.bash` | PSF + OTF + MTF demo. Default lens `my-schmidt.yaml` (the DLS+escape-optimised Schmidt, fields 0/1/2/2.8°): a single `rayweave psf` run (RCP+LCP, `--max-freq 100` cap) computes the image-plane PSF and the FFT-derived OTF/MTF, then prints a Strehl / FWHM / EE50 / Airy / Grid / MTF50-30-10 table (`<stem>-result.txt`) and draws per-field pm3d maps, a radial-profile overlay, and a sagittal/tangential MTF-overlay PNG. `--lens triplet` (center-field `psf-mtf-demo.yaml`), `--lens doublegauss`, or any YAML path switches the lens — the MTF cap keeps working via `--max-freq`, which overrides `psf.mtf_config.max_frequency` even when the chosen YAML has no `psf:` section. |
