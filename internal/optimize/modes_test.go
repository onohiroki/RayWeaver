...
func TestGlassRoleDLSRecoversPositiveFlint(t *testing.T) {
	...
	opt := NewMultiOptimizer([]ConfigInput{cfg}, nil, localVars, gc, 100, 0.01, 1e-6, 1e-4, 1.0, 16, 0, 0, nil, nil, 0, 0, false, false, false)
	res := opt.Optimize()
...
func TestMeritScheduleBlendLeastSquares(t *testing.T) {
	...
	opt := NewMultiOptimizer(configs, nil, nil, tripletGC(), 1, 0.01, 1e-6, 1e-6, 1.0, 32, 0, 0, nil, nil, 0, 0, false, false, false)
...
func TestMeritScheduleStepIsHardSwitch(t *testing.T) {
	...
	refOpt := func(mode string) *Optimizer {
		...
		return NewMultiOptimizer([]ConfigInput{ref}, nil, nil, tripletGC(), 1, 0.01, 1e-6, 1e-6, 1.0, 32, 0, 0, nil, nil, 0, 0, false, false, false)
	}
...
func TestMeritScheduleDLSRecoversGlassRoles(t *testing.T) {
	...
	opt := NewMultiOptimizer([]ConfigInput{cfg}, nil, localVars, gc, 80, 0.01, 1e-6, 1e-4, 1.0, 32, 100, 0, nil, nil, 0, 0, false, false, false)
	opt.SetMeritSchedule(schedule)
	res := opt.Optimize()
