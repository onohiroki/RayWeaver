...
`psf` (`rayweaver psf [--ref-surface N] [--psf-grid 64] [--psf-width W] [--num-rays 400] [--fields I,...] [--wavelengths W,...] [--polarization RCP|LCP|X|Y|RCP+LCP] [--best-focus] [--converge-check] [--converge-tol T] [--max-freq N] [--yaml FILE] [--csv FILE]`) computes the point-spread function on the **fixed flat image plane** via per-field polarized ray tracing, non-uniform wavefront sampling (Delaunay-triangulated reference surface) and a **direct vector Huygens integral** — no FFT, fisheye-safe. Requires a `chief` section (fields). It reuses the chief dynamic-pupil / stop grid to sample each field's entrance pupil, traces the polarized wavefront to the reference surface (default: the last optical surface) with full Jones tracking (`SurfaceResult.Field`), weights the samples by 3D Delaunay triangle areas (`internal/mesh`), and coherently sums secondary wavelets `E(P) = Σ E_j·exp(ik(OPL_j + n·R_j))·K_j·ΔA_j/R_j`. `--best-focus` (or `psf.best_focus`) evaluates each field at its **best-focus image plane** instead: the plane shift minimizing the geometric spot RMS (`internal/psf`, `BestFocusShift`, golden-section `minimize1D` over `ImagePlaneSpot`) is applied per field before the Huygens integral, removing field-curvature defocus — the applied shift is reported per result as `best_focus_shift_mm`, and the Strehl then matches the `wavefront` command's best-focus value. The image grid is **auto-enlarged** when the requested grid would leave the Airy core under a pixel (fast/aberrated systems where the geometric spot dominates the window): the peak-ratio Strehl is only meaningful when the ideal reference core is resolved, so the grid is raised until the pixel pitch ≤ Airy radius/2 (the effective size is reported in `psf_results[].grid_size`). The pipeline YAML carries a lightweight `psf_results[]` summary (Strehl vs a same-samples diffraction-limited reference, FWHM, centroid, encircled-energy 50%, Airy radius, sampling counts, plus the MTF sagittal/tangential threshold and evaluated-frequency summary); full grids (intensity + Ex/Ey/Ez, encircled-energy curve, best-fit-sphere wavefront `rms_opd`/`pv_opd`) and the OTF/MTF curves go to `--yaml` files and gnuplot pm3d maps (blank-line-separated rows, `set datafile separator ","`) to `--csv` files, one index-suffixed file per result. `--max-freq N` caps the reported MTF curves and threshold search at N cycles/mm (default `psf.mtf_config.max_frequency`, else the Nyquist). Default input polarization is RCP; `RCP+LCP` averages intensities incoherently. Sampling **convergence** is labelled by default (disable with `--converge-check=false` or `psf.converge_check: false`; threshold `--converge-tol T` or `psf.converge_tol`, default 0.10): each (field, wavelength, polarization) result is re-evaluated at 1.5× the ray count and the relative Strehl change is reported per result as `psf_results[].converged` / `strehl_rel_change` / `check_rays` — strongly aberrated or speckle PSFs stay labelled unreliable until `--num-rays` is raised, and `converge_check` costs roughly 1.5× the run time. See `docs/psf.md` and `docs/methods/psf.md`. Off-axis fields on a well-corrected lens reach near-diffraction-limited Strehl (the wavefront-plane launch removed the OPL tilt that previously collapsed them to ~0.03); genuinely aberrated off-axis fields give speckle PSFs whose Strehl is sampling-sensitive — raise `--num-rays` (900..1600) for those.

`wavefront` (`rayweaver wavefront [--ref-surface N] [--num-rays 400] [--fields I,...] [--wavelengths W,...] [--polarization S] [--zernike-order N] [--wavefront-workers N] [--map-grid 64] [--best-focus] [--focus-weight uniform|custom] [--focus-weights W,...] [--output-shifted-lens FILE] [--yaml FILE] [--csv FILE]`) analyses the wavefront on the reference surface (default: the last optical surface) per (field, wavelength, polarization). Requires a `chief` section (fields). It traces each field's entrance pupil with the same dynamic-pupil grid, references the OPD to the **best-focus point** (`OPL + n·|P−Fbest|`, `psf.ImagePlaneSpot`, sphere center from the spot-RMS minimization), and produces three descriptions: an always-computed **paraboloid** fit (`ax²+by²+cxy+dx+ey+f` + derived defocus/astigmatism/tilt, `internal/wavefront`), a **best-fit sphere** whose focus minimizes the **geometric spot RMS** along the image-plane normal, and a **stabilized Fringe-Zernike** decomposition (1..37, Zemax convention) of the low-order-removed residual. `--best-focus` combines the per-field shifts by weight (`uniform` default, `custom` with `--focus-weights`) and applies the weighted average to each target config's image-plane last-decenter `shift[2]` (per-config image distance recomputed), so `chief | wavefront --best-focus | psf` computes a best-focus PSF; `--output-shifted-lens FILE` writes the shifted lens standalone. Pipeline YAML carries a lightweight `wavefront_result` (coefficients only; `output_file` references the `--yaml` file with scattered samples + interpolated residual-OPD map; `--csv` writes the pm3d map). The statistics `rms`/`pv`/`strehl` use the reference-sphere residual (piston+tilt+defocus removed, astigmatism retained) and match `psf --best-focus`'s `rms_opd`/`pv_opd`/`strehl_ratio`; `strehl` is the **exact peak ratio `|<e^{i(2π/λ)W}>|²`** (pupil-area-weighted coherent average of the residual wavefront `W`, `internal/wavefront.exactStrehl`), meaningful beyond the Maréchal limit (~0.2 λ) where `exp(−(2πσ/λ)²)` collapses towards zero. The paraboloid `rms_residual` additionally removes astigmatism. See `docs/wavefront.md`.
...
- `import` / `export` read/write a foreign format (not YAML) — no input-YAML settings
  to overwrite (`export --format zemax|codev|oslo [--config ID] [--nd-vd] [-o FILE]` writes
  ZMX / SEQ / LEN text; the config policy: ZEMAX/CODE V export every config — as
...
- **Off-axis spot merit kinds** (`spot_rms_t`/`spot_rms_s`/`spot_rms_worst`/`spot_rms_weighted`/`spot_ee_radius`): beyond the rotationally symmetric, uniformly-weighted `spot_rms`, these reuse the same pupil grid but weight each ray by its **pupil-cell area × transmitted intensity** (`IPoint.Area`/`Intensity`, set in `dls.traceGridRays`; weight = area×intensity, degrading to area, then to equal weight). `spot_rms_t`/`_s` decompose the deviation into the field's tangential (image-plane azimuth from `fields[].direction`) and sagittal axes — a comatic spot has RMS_T ≫ RMS_S; `spot_rms_worst` = max of the two; `spot_rms_weighted` is the flux-weighted RMS; `spot_ee_radius` is the encircled-energy radius about the flux-weighted centroid (`fraction` on the merit term, default 0.8). All five contribute `(value−target)²`. Motivation: spot RMS alone cannot separate coma/astigmatism/field-curvature, is tail-dominated, and ignores vignetting/reflection weighting, which caps off-axis-field improvement.
...
- **Wavefront-plane launch (angle fields)**: angle-field rays are launched parallel from the **wavefront plane perpendicular to the ray direction** (`raymath.ProjectOntoWavefront`), not from the optical-axis-perpendicular plane z=-100. Each launch origin is projected onto the wavefront through the grid centre along the ray direction, so the OPL is referenced to a common wavefront and carries **no launch-geometry tilt** (previously the OPL had a linear `Δpx·sinθ` ramp that polluted off-axis OPD and shifted the Huygens PSF peak). The projection is along `rayDir` so ray lines (and every surface intersection / pupil footprint) are unchanged. Grid centring uses `raymath.WavefrontGridCenter` (direction-vector ratios, no `tanθ`), so it stays finite up to 90° incidence (at grazing angles the grid centre falls back to the entrance-pupil centre). Applied in `tracePupilGrid`/`computeRayFan`/`marginalsAtStop` (chief), `traceGridRays` (dls), `frozenPupilGrid` (wavefront), `GenerateFootprints` (asphere). Height-based (finite-conjugate `height`+`object_z`) fields keep the object-point launch and are unaffected.
...
| `wavefront-demo.bash` | Wavefront analysis of the US2645157 triplet (0/16/24°): paraboloid (defocus/astig/tilt) + stabilized Fringe-Zernike (dominant terms) + statistics per field, interpolated OPD maps (pm3d), and the best-focus correction (`--best-focus` applied, re-analysis shows the on-axis defocus vanish). `--lens doublegauss` switches to the 4-field double-Gauss (`doublegauss-init.yaml`, ~8 mm out of focus); `--defocus D` (requires yq) shifts the image plane +D mm first so the correction is dramatic. d-line only. `--clean` removes all artifacts |
| `psf-mtf-demo.bash` | PSF/OTF/MTF demo on the escape-optimised US2645157 triplet refined with the 16° wavefront_astigmatism merit (w16=14000, 24° spot terms w24=0.05, `psf-mtf-demo.yaml`): per-field intensity maps + radial/MTF overlays + comparison table. The fields show a monotone 0° > 16° > 24° Strehl profile — 0° ~0.99 near-diffraction-limited (MTF50 ~104 c/mm), 16° ~0.62 (MTF50 ~70 c/mm), 24° ~0.59 and the lowest peak (MTF50 ~51 c/mm) — meeting 0.9/0.6/0.5 for the demo. `--lens doublegauss|path` substitutes another lens; the MTF cap is `--max-freq` (default 200 c/mm). `--clean` removes artifacts |
