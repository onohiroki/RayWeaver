...
| `schmidt-psf-demo.bash` | PSF/OTF/MTF of the folded D=200 F/1.93 Schmidt (`schmidt-flattener.yaml`) at **all four fields 0/1/2/3.22°** (35 mm full-frame): per-field intensity maps + radial/MTF overlays + comparison table. Shows the auto-enlarged image grid (requested 96 → effective ~180-450, printed in the Grid column) and the λ/4-residual Strehl/MTF far below the ~880 c/mm diffraction cut-off. `--lens triplet|doublegauss|path` substitutes another lens; `--clean` removes artifacts |
