...
| `psf-mtf-demo.bash` | PSF/OTF/MTF demo on the escape-optimised US2645157 triplet restricted to the center field (`psf-mtf-demo.yaml`): per-field intensity maps + radial/MTF overlays + comparison table. The 0° field is nearly diffraction-limited (Strehl ~0.87, MTF50 ~80 c/mm). `--lens doublegauss|path` substitutes another lens; the MTF cap is `--max-freq` (default 200 c/mm). `--clean` removes artifacts |
