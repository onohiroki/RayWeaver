...
| `asphere-demo.bash` | Adds one asphere to an optimised all-spherical double-Gauss (`asphere-demo-init.yaml`): per case ranking → spot-RMS validation → `--apply` → spot RMS before/after → PNG diagrams + per-case gate. Always runs two cases — the full-aperture lens and a stopped-down variant (entrance pupil diameter halved, or `--epd N` mm, surface 7's fixed aperture resized via yq) — then draws a single **side-by-side** OPD-overlap chart (left = full aperture, right = stopped-down) whose y-range is shared across BOTH cases, and prints a full-vs-stopped spot-RMS comparison table. `--clean` removes tagged `-half`/`-epdN` artifacts too |
