...
| `psf-mtf-demo.bash` | PSF/OTF/MTF demo on the escape-optimised US2645157 triplet refined with the 16° wavefront_astigmatism merit (`psf-mtf-demo.yaml`): per-field intensity maps + radial/MTF overlays + comparison table. The 0° field is nearly diffraction-limited (Strehl ~0.98, MTF50 ~100 c/mm), the 16° field improves to Strehl ~0.14, and the 24° field reaches Strehl ~0.95 (MTF50 ~91 c/mm). `--lens doublegauss|path` substitutes another lens; the MTF cap is `--max-freq` (default 200 c/mm). `--clean` removes artifacts |
