...
| `psf-mtf-demo.bash` | PSF/OTF/MTF demo. Default lens is the DLS+escape-optimised folded D=200 F/1.93 Schmidt (`my-schmidt.yaml`) at its usable fields 0/1/2/2.8° (the legacy 3.22° full-frame edge vignettes and is dropped): per-field intensity maps + radial/MTF overlays + comparison table with a Grid column (auto-enlarged image grid, requested 96 → effective ~180-450, when the geometric spot dominates the Airy core) and the λ/4-residual Strehl/MTF far below the ~880 c/mm diffraction cut-off. `--lens triplet|doublegauss|path` substitutes another lens (the triplet switches back to the center-field `psf-mtf-demo.yaml`); the MTF cap is `--max-freq` (default 100 c/mm). `--clean` removes artifacts |
