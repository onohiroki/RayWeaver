...
- `SIGINT`/`SIGTERM` stop `escape` in three escalating stages (handler in `runEscape`), each one still completing normally (`interrupted: true`, exit 0) except the last:
  ...
  `Cycle.StoppedByTime()` and `ParallelEscape`'s `TimedOut` exclude both ctx cancellation and a hard stop. The `optimize` command never sets `Options.Stop` (nil), so mid-solve interruption only exists for `escape`.
